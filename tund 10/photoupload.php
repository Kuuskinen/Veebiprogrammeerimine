<?php
	...
	$teine = new Photoupload("Ja nii juba teist korda. ");*/
	...
	if(isset($_POST["submit"])) {
		...
		if(!empty($_FILES["fileToUpload"]["name"])){
			...
			/*if (file_exists($target_file)) {
				...
				$uploadOk = 0;
			}*/ //ei ole enam mõtet kontrollida, sest pildid saavad nimeks ajatempli, millel nanosekundi täpsus
			...
			// Kui kõik on OK, try to upload file
				...
				$myPhoto->addTextWatermark("Unistus kevadest");
				$notice = $myPhoto->savePhoto($target_dir, $target_file); //tahab directory kuhu laadida ja filename mida laadida
				//$myPhoto->saveOriginal($directory, $target_file);
				$myPhoto->clearImages();//selle mõttetuks
				
				unset($myPhoto); //see muudab
				
				/*if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"]<--ajutine fail!, $target_file)) { //läbis kontrolli ja move'is selle algsuuruses serveri kataloogi
                ...
                //$textToImage = "Heade mõtete veeb"; SEDA EI TAHA				
			    //määrata värv
